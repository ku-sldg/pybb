...
const ACTS = [
  ...
  { num: "ACT II", title: "Spec drift: benign, promote then re-verify", scenes: "scene 3 (expand allowed temperature range)",
    watch: "A benign change in requirements — the temperature alarm range widened — model appraisal fails quickly.  Administrator re-blesses and promotes new spec, all contract appraisals again pass.",
  ...
  { num: "ACT III", title: "Implementation drift: benign, re-verify", scenes: "scene 13",
    watch: "A developer rewrites implementation logic — semantically equivalent. The hash moves, but every contract slice maintains integrity, and the proofs re-verify: the benign change survives.",
  ...
  { num: "ACT IV", title: "Contract drift: breaking, restore then attempt re-verification", scenes: "scene 14",
    watch: [
      ...
      { text: "codegen", options: { bold: true } },
      { text: ", but before verification", options: { bold: true } },
      { text: ") and the code is inverted to match — ", options: { italic: true } },
      { text: "verus", options: { italic: true } },
      { text: " checks pass.  Only the contract tier sees the drift; restoring the true contract exposes the Verus refusal the laundering hid.", options: { italic: true } },
    ],
  ...
  { num: "ACT VI", title: "Trust-anchor tamper: attribute then refuse", scenes: "scenes 6 + 7",
    watch: "The signed evidence, an installed golden, then the verifier itself — each tamper attributed, each refused. No knowledge source repairs a trust anchor.",
  ...
  { num: "ACT VII", title: "Verification soundness", scenes: "scenes 9 + 12",
    watch: "Proofs verify, but measurement detects unsound axioms.",
