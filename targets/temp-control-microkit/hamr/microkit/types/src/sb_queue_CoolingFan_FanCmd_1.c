...
// Sender API
//
// See sb_queue_CoolingFan_FanCmd_1.h for API documentation. Only implementation details are documented here.

void sb_queue_CoolingFan_FanCmd_1_init(sb_queue_CoolingFan_FanCmd_1_t *queue) {
...
void sb_queue_CoolingFan_FanCmd_1_enqueue(
  sb_queue_CoolingFan_FanCmd_1_t *queue,
  CoolingFan_FanCmd *data) {

  // Simple ring with one dirty element that will be written next. Only one
  // writer, so no need for any synchronization.
  // elt[queue->numSent % SB_QUEUE_COOLINGFAN_FANCMD_1_SIZE]
  // is always considered dirty. So do not advance queue->NumSent
  // till AFTER data is copied.

  size_t index = queue->numSent % SB_QUEUE_COOLINGFAN_FANCMD_1_SIZE;
...
void sb_queue_CoolingFan_FanCmd_1_Recv_init(
  sb_queue_CoolingFan_FanCmd_1_Recv_t *recvQueue,
  sb_queue_CoolingFan_FanCmd_1_t *queue) {
...
bool sb_queue_CoolingFan_FanCmd_1_dequeue(
  sb_queue_CoolingFan_FanCmd_1_Recv_t *recvQueue,
  sb_event_counter_t *numDropped,
  CoolingFan_FanCmd *data) {

  sb_event_counter_t *numRecv = &recvQueue->numRecv;
  sb_queue_CoolingFan_FanCmd_1_t *queue = recvQueue->queue;
  ...
  // incremented. Thus there are really only (SB_QUEUE_COOLINGFAN_FANCMD_1_SIZE - 1)
  // elements in the queue.
  *numDropped = (numNew <= SB_QUEUE_COOLINGFAN_FANCMD_1_SIZE - 1) ? 0 : numNew - SB_QUEUE_COOLINGFAN_FANCMD_1_SIZE + 1;
  ...
  *numRecv += *numDropped + 1;
  ...
  size_t index = (*numRecv - 1) % SB_QUEUE_COOLINGFAN_FANCMD_1_SIZE;
  *data = queue->elt[index]; // Copy data

  // Acquire memory fence - ensure read of data BEFORE reading queue->numSent again
  __atomic_thread_fence(__ATOMIC_ACQUIRE);

  if (queue->numSent - *numRecv + 1 < SB_QUEUE_COOLINGFAN_FANCMD_1_SIZE) {
...
bool sb_queue_CoolingFan_FanCmd_1_is_empty(sb_queue_CoolingFan_FanCmd_1_Recv_t *recvQueue) {
