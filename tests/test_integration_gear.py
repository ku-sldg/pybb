...
PROTOCOL_IDS = ("landing_gear_lean_model", "landing_gear_lean_contracts")
TIER_IDS = ("landing_gear_lean_verification", "landing_gear_lean_executable")
...
def _episode(repair: bool = True):
    ...
    chain = [TierKS(protocol_id="landing_gear_lean_contracts")]
    if repair:
        chain.append(WholeFileRestoreKS(golden_root=GOLDEN_ROOT,
                                        refined_by="landing_gear_lean_contracts"))
    for ks in chain:
        ctl.add_ks(ks)
    ctl.blackboard.write_entry(key="landing_gear_lean:model", predicate="attestation",
                               measurement=attestation_request("landing_gear_lean_model"))
    ctl.route("landing_gear_lean:model", on_fail=chain)
    ctl.run()
...
def test_clean_attestation_of_gear_package():
    bb = _episode()
    entry = bb.get_entry("landing_gear_lean:model")
    assert entry.good_standing and entry.result.protocol == "landing_gear_lean_model"
...
def test_interlock_tamper_attributed_by_name_repaired_verified():
    ...
    args = _protocols()["landing_gear_lean_contracts"].asp_args["readfile_range"][targ]
    ...
    try:
        bb1 = _episode()
        escalated = bb1.escalate["landing_gear_lean:model"]
        assert escalated.ks_history == {"tier:landing_gear_lean_contracts": 1, "repair:whole-file": 1}
        l2v = next(e.result for k, e in bb1.get_history()
                   if k == "landing_gear_lean:model" and e.result is not None
                   and e.result.protocol == "landing_gear_lean_contracts")
        ...
        bb2 = _episode()
        assert bb2.entries["landing_gear_lean:model"].good_standing
...
def test_am_detection_and_expecteds_are_config_driven():
    ...
    protocols = _protocols("landing_gear_lean_model")
    assert not changed_decls(protocols["landing_gear_lean_contracts"], GEAR_ROOT, prefix="landing_gear_lean",
                             props_protocol=protocols["landing_gear_lean_model"])
    sys.path.insert(0, str(REPO / "examples"))
    from landing_gear_lean import EXEC_KEYS, resolved_exec_targets, vector_failures
    targets = resolved_exec_targets()
    assert [targets[f"landing_gear_lean_executable_{k}_targ"]["expected"] for k in EXEC_KEYS] \
    ...
    failures = vector_failures(
        ...
        a["exe_args"] else targets[f"landing_gear_lean_executable_airborne_targ"]["expected"]
        if a["exe_args"][-2] == "Up" else "gearCmd=Extend")
    assert failures == ["landing_gear_lean_executable_ground_targ: expected 'gearCmd=Hold', "
                        "got 'gearCmd=Retract'"]
...
def test_proofs_and_behavior_tiers_clean_with_woven_tools():
    check = _tier_verdict("landing_gear_lean_verification")
    assert check.passed, check
    tool_targs = {c.targ_id for c in check.components
                  if (c.args.get("metadata") or "").startswith("tool::lean")}
    assert len(tool_targs) == 6
    behavior = _tier_verdict("landing_gear_lean_executable")
    assert behavior.passed, behavior
    assert {"landing_gear_lean_executable_ground_targ", "landing_gear_lean_executable_airborne_targ",
            "landing_gear_lean_executable_extend_targ"} <= {c.targ_id for c in behavior.components}
