...
provisioned temp_control_aadl_slang_l1a/temp_control_aadl_slang_l1b/temp_control_aadl_slang_l2 protocols for temp-control-jvm on
...
safe-to-edit files) but fails the temp_control_aadl_slang:contracts entry at block
granularity.
...
def _run():
    """Both trust questions: temp_control_aadl_slang:files (l1a -> l2 on fail) and
    temp_control_aadl_slang:contracts (l1b, block-granular, no deeper dive)."""
    ...
    ctl.route("temp_control_aadl_slang:files", on_fail=[refine])
    ctl.route("temp_control_aadl_slang:contracts", on_fail=[])
    ctl.run()
...
def test_clean_run_passes_no_escalation():
    ...
    files = bb.get_entry("temp_control_aadl_slang:files")
    assert files is not None and files.good_standing
    assert files.result.protocol == "temp_control_aadl_slang_l1a"
    assert len(files.result.components) == 5  # 4 hashfile_appr + sig_appr
    assert files.ks_history == {}  # l2 rung never fired
    contracts = bb.get_entry("temp_control_aadl_slang:contracts")
...
def test_tampered_contract_fails_l1_and_l2_attributes(live_snapshot):
    ...
    assert "temp_control_aadl_slang:files" not in bb.entries and "temp_control_aadl_slang:files" in bb.escalate
    escalated = bb.escalate["temp_control_aadl_slang:files"]
    assert escalated.ks_history == {"tier:temp_control_aadl_slang_l2": 1}
    l2 = escalated.result
    assert l2.protocol == "temp_control_aadl_slang_l2" and not l2.passed
    ...
    assert bb.get_entry("temp_control_aadl_slang:contracts").good_standing
...
def test_block_tamper_invisible_to_l1a_caught_by_l1b(live_snapshot):
    ...
    assert bb.get_entry("temp_control_aadl_slang:files").good_standing

    # the contracts sentinel catches it at block granularity
    assert "temp_control_aadl_slang:contracts" in bb.escalate
    l1b = bb.escalate["temp_control_aadl_slang:contracts"].result
    assert l1b.protocol == "temp_control_aadl_slang_l1b" and not l1b.passed
