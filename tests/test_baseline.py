...
Runs against the committed temp_control_lean_model/temp_control_lean_contracts bundles; auto-skipped unless
...
def test_clean_bundles_verify_and_anchor_every_golden():
    for pid, n_targets in (("temp_control_lean_model", 2), ("temp_control_lean_contracts", 15)):
...
def test_hand_edited_golden_detected_and_attributed():
    ...
    edited target."""
    proto = _load("temp_control_lean_contracts")
...
def test_tampered_bundle_fails_signature():
    ...
    bundle = GOLDEN_ROOT / "_bundles" / "temp_control_lean_model" / "provision_bundle.json"
    ...
    try:
        report = verify_bundle(_client(), _load("temp_control_lean_model"), GOLDEN_ROOT)
        assert not report
    ...
    assert verify_bundle(_client(), _load("temp_control_lean_model"), GOLDEN_ROOT)


def test_missing_bundle_is_a_problem():
    proto = _load("temp_control_lean_model").model_copy(update={"protocol_id": "no_such_protocol"})
    report = verify_bundle(_client(), proto, GOLDEN_ROOT)
    assert not report
...
def test_target_map_drift_requires_reprovisioning():
    ...
    proto = _load("temp_control_lean_model")
...
def test_readiness_verifies_signed_baselines():
    ...
    protocols = {pid: _load(pid)
                 for pid in ("temp_control_lean_model", "temp_control_lean_contracts", "temp_control_lean_verification", "temp_control_lean_executable")}
    ...
    assert report.baseline_verified == ["temp_control_lean_model", "temp_control_lean_contracts",
                                        "temp_control_lean_verification", "temp_control_lean_executable"]


def test_readiness_reports_baseline_integrity_as_distinct_category():
    from pybb.attestation import make_readiness_predicate, readiness_request

    protocols = {"temp_control_lean_contracts": _load("temp_control_lean_contracts")}
    protocols["temp_control_lean_contracts"].asp_args["readfile_range"][
        "temp_control_lean_spec_fanOn_when_hot_targ"]["golden_b64"] = "Zm9yZ2Vk"
    predicate = make_readiness_predicate(protocols, baseline_root=GOLDEN_ROOT,
                                         client=_client())
    report = predicate(readiness_request(["temp_control_lean_contracts"]))
    assert not report
...
def test_readiness_without_baseline_root_unchanged():
    ...
    protocols = {"temp_control_lean_model": _load("temp_control_lean_model")}
    report = make_readiness_predicate(protocols)(readiness_request(["temp_control_lean_model"]))
    assert report
