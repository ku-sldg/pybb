...
Runs against the committed lean_l1a/lean_l2 bundles; auto-skipped unless
...
def test_clean_bundles_verify_and_anchor_every_golden():
    for pid, n_targets in (("lean_l1a", 6), ("lean_l2", 15)):
...
def test_tampered_bundle_fails_signature():
    ...
    bundle = GOLDEN_ROOT / "_bundles" / "lean_l1a" / "provision_bundle.json"
    ...
    try:
        report = verify_bundle(_client(), _load("lean_l1a"), GOLDEN_ROOT)
        assert not report
    ...
    assert verify_bundle(_client(), _load("lean_l1a"), GOLDEN_ROOT)


def test_missing_bundle_is_a_problem():
    proto = _load("lean_l1a").model_copy(update={"protocol_id": "no_such_protocol"})
    report = verify_bundle(_client(), proto, GOLDEN_ROOT)
    assert not report
...
def test_readiness_verifies_signed_baselines():
    ...
    protocols = {pid: _load(pid)
                 for pid in ("lean_l1a", "lean_l2", "lean_check", "lean_exec")}
...
def test_readiness_reports_baseline_integrity_as_distinct_category():
    ...
    protocols = {"lean_l2": _load("lean_l2")}
    protocols["lean_l2"].asp_args["readfile_range"][
        "lean_spec_fanOn_when_hot_targ"]["golden_b64"] = "Zm9yZ2Vk"
    predicate = make_readiness_predicate(protocols, baseline_root=GOLDEN_ROOT,
                                         client=_client())
    report = predicate(readiness_request(["lean_l2"]))
    assert not report
    assert not report.problems  # config is fine — this is a BASELINE failure
    assert any("lean_spec_fanOn_when_hot_targ" in p
...
def test_readiness_without_baseline_root_unchanged():
    ...
    protocols = {"lean_l1a": _load("lean_l1a")}
    report = make_readiness_predicate(protocols)(readiness_request(["lean_l1a"]))
    assert report
